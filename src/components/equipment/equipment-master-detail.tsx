...
/**
 * Stacked master-detail for a customer's equipment (Pattern B, see
 * `docs/design/master-detail-pattern.md`). Top = the equipment list in a
 * fixed-height (~10 rows), independently-scrolling region with a sticky header;
 * clicking a row selects+highlights it (no inline accordion). Below the list a
 * separate detail section shows the selected unit's panel (5 tabs), or a
 * "select a unit" placeholder. Bottom = the function bar. The selected unit
 * lives in the URL (`?tab=equipment&equipmentId=…`) so it survives back/forward
 * and deep links, matching the tab-sync pattern the customer page already uses.
 */
...
export function EquipmentMasterDetail({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Equipment list — fixed height (~10 rows), sticky header, own scroll.
          Row click selects+highlights; the detail renders in its own section
          below (no inline accordion). */}
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-[#e5e5e5] bg-[#fafafa] p-6 text-center text-sm text-[#737373]">
          {t("noEquipment")}
        </div>
      ) : (
        <div className="max-h-[24rem] overflow-auto rounded-xl border border-[#e5e5e5] bg-white">
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10 bg-[#fafafa] text-[#525252]">
              <tr>
      ...
      {/* Detail section — below the list, for the selected unit. */}
      {selectedId ? (
        ...
        <div className="rounded-xl border border-dashed border-[#e5e5e5] bg-[#fafafa] p-8 text-center text-sm text-[#737373]">
