...
/**
 * The one body eSMS has registered for brandname `JAKE'S HOME APPLIANCES` (ViHAT sheet
 ...
 *   - No `[JakeApp]` prefix: the brandname is already the sender ID.
...
const SMS_VISIT_REMINDER: TemplateDef = {
  ...
  bodies: smsBodies(
    "TB BAO TRI DINH KY: KTV cua JAKE'S HOME APPLIANCES du kien se den bao tri {equipment} cua QK vao {datetime}. Neu QK can doi khung gio khac vui long LH: 0768902009.",
    "MAINTENANCE NOTICE: JAKE'S HOME APPLIANCES technician will service your {equipment} on {datetime}. To reschedule, please contact 0768902009.",
  ),
};
...
const SMS_SR_APPROVED: TemplateDef = {
  ...
  category: "TRANSACTIONAL",
  bodies: smsBodies(
    "[JakeApp] YC #{req_no} duyet. Chi phi: {amount}d - Hen: {date}. XN: soms.jakeshomeappliances.com.vn",
    "[JakeApp] Request #{req_no} approved. Cost: {amount} VND - Visit: {date}. Confirm: soms.jakeshomeappliances.com.vn",
  ),
};
...
const SMS_SR_REJECTED: TemplateDef = {
  ...
  category: "TRANSACTIONAL",
  bodies: smsBodies(
    "[JakeApp] YC #{req_no} tu choi. Ly do: {reason}. LH {hq_phone}",
    "[JakeApp] Request #{req_no} declined. Reason: {reason}. Contact {hq_phone}",
  ),
};
...
const SMS_PAYMENT_OVERDUE_FINAL: TemplateDef = {
  ...
  category: "TRANSACTIONAL",
  bodies: smsBodies(
    "[JakeApp] {name}, phi thue {month} {amount}d chua TT. TT: soms.jakeshomeappliances.com.vn hoac {hq_phone}",
    "[JakeApp] {name}, {month} rental {amount} VND overdue. Pay soms.jakeshomeappliances.com.vn or {hq_phone}",
  ),
};
...
const SMS_CONTRACT_RENEWAL_FINAL: TemplateDef = {
  ...
  category: "TRANSACTIONAL",
  bodies: smsBodies(
    "[JakeApp] {name}, HD thue het han {date} (con {days} ngay). Chuyen SH/bao tri: soms.jakeshomeappliances.com.vn / {hq_phone}",
    "[JakeApp] {name}, rental ends {date} ({days} days left). Transfer/maintenance: soms.jakeshomeappliances.com.vn / {hq_phone}",
  ),
};
