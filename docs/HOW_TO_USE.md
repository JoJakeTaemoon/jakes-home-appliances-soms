...
**누가 읽어야 하나요?** Seoul Aqua에서 일하시는 분, 또는 Seoul Aqua의 고객 — 모두입니다.
...
Seoul Aqua는 정수기, 공기청정기, 비데를 **팔고, 빌려주고, 정기적으로 관리해 드리는** 회사예요. 손님이 한두 분이면 종이 장부로 충분하지만, 손님이 수백 명이고 기사님이 수십 분이 되면 종이로는 안 됩니다.
...
1. **컴퓨터를 켜고** SOMS 사이트(예: `soms.seoulaqua.com.vn/o/login`)로 들어가요
...
1. **SOMS 모바일** 사이트(예: `soms.seoulaqua.com.vn/f/login`)로 들어가요
...
1. Seoul Aqua 영업 직원과 상담
2. 계약서에 서명
3. **휴대폰으로 SMS 한 통**이 와요:
   > 안녕하세요. Seoul Aqua 고객 포털 가입이 완료되었습니다.
   > 임시 비밀번호: ********
   > 로그인: `seoulaqua.com.vn/login`
...
- **하루 전(D-1)**에 SMS 알림이 와요:
  > 내일 09~11시 정수기 정기점검 예정입니다. (Seoul Aqua)
...
1. **Seoul Aqua 사무실로 전화**하세요 (로그인 화면에 스스로 재발급받는 버튼은 없습니다)
2. 사무실이 본인 확인 후 재설정해 줍니다
...
**또는 Seoul Aqua 사무실 MANAGER에게 전화**로 부탁해도 돼요.
...
> 비밀번호 잊으면 ADMIN 또는 MANAGER에게 전화하세요. 임시 비밀번호를 그 자리에서 발급해 알려줍니다.
...
**비밀번호 잊었어요** → ADMIN 또는 MANAGER에게 전화 (관리자 → 사용자 관리 → 비밀번호 초기화)
