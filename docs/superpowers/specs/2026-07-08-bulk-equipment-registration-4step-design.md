...
- **상태**: 설계 확정 (스키마 변경 승인됨)
- **부분 폐기 (2026-09-04)**: 관리번호(`assetCode`)의 "자동/직접입력" 선택은 폐기되었습니다. 장비코드는 등록 경로와 무관하게 서버가 자동 발행합니다 (형식은 2026-09-07 `MAY-{NNNNNN}` 모델별 순번으로 재변경) — `docs/SPEC.md` §4.3.1 참고. 아래 관리번호 관련 서술은 당시 설계 기록으로만 유효합니다.
...
| 개월→일 계수 | **30 고정** |
| 관리번호 | 스펙의 "장비 관리번호" = `assetCode`. ~~자동/직접입력~~ → **서버 자동 발행 전용** (2026-09-07 현행 규칙은 SPEC §4.3.1 — `MAY-{NNNNNN}` 모델별 순번). `serialNumber`는 기존대로 자동 백그라운드 생성 |
...
- ~~관리번호 방식: `자동생성`/`직접입력` (자동시 미리보기 표시)~~ → **폐기(2026-09-04)**. `assetCode`는 제출 시 서버가 발행하므로 화면에는 발행 규칙 안내만 표시
...
- **스텝1 고객 선택**: 동일
- **스텝2 장비 정보**: 단일 모델 대신 **모델 라인 여러 개 추가**(라인 = 모델 + 수량 + 시리얼 입력 방식 + (사이트 고객시) 설치 위치). 설치일·담당 기사는 배치 공통 or 라인별(기본 공통)
