...
- **상태**: 설계 확정 (스키마 변경 승인됨)
- **부분 폐기 (2026-09-04)**: 관리번호(`assetCode`)의 "자동/직접입력" 선택은 폐기되었습니다. 장비코드는 등록 경로와 무관하게 서버가 `{modelCode}{YY}{MM}{DD}{NNNN}` 규칙으로 자동 발행합니다 — `docs/SPEC.md` §4.3.1 참고. 아래 관리번호 관련 서술은 당시 설계 기록으로만 유효합니다.
...
| 개월→일 계수 | **30 고정** |
| 관리번호 | 스펙의 "장비 관리번호" = `assetCode`. ~~자동/직접입력~~ → **서버 자동 발행 전용**(2026-09-04, SPEC §4.3.1). `serialNumber`는 기존대로 자동 백그라운드 생성 |
