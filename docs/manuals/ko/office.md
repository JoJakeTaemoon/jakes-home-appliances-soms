...
**보유 장비 탭은 한 화면 마스터-디테일입니다(개편).** 왼쪽에 장비 목록(검색·사업장 필터), 오른쪽에 선택한 장비의 상세가 함께 표시됩니다. 목록에서 행을 클릭하면 **페이지 이동 없이** 오른쪽 패널이 바뀝니다.

- **우측 상세 = 탭 5개**: 기본정보 · 필터정보 · 서비스이력 · 수금내역 · 메모. 기본정보 탭에 서비스 구성 통합표(아래)와 주요정보·다음 일정이 모여 있습니다.
- **하단 기능버튼**: 신규 등록(F2)·수정(F5)·상세(전체 화면)·닫기. 신규 등록·수정은 **매니저 이상** 권한에서 보입니다.
- **전체 화면 보기**: 무거운 작업(이전·교체·해지·회수)이 필요하면 우측 패널의 **"전체 화면"** 링크로 기존 **전용 장비 상세 페이지**(§7.3)를 엽니다. 기존 링크·딥링크·뒤로 가기도 그대로 동작하며, 뒤로 가기는 장비 탭에 머뭅니다.
- 선택한 장비는 주소(URL)에 기록되어 새로고침·뒤로 가기에도 유지됩니다.
...
> **화면 구성(마스터-디테일)**: 모델·소모품 탭은 **좌측 편집 폼 / 우측 목록(표)** 구조입니다. 우측 표에서 행을 클릭하면 좌측 폼에 불러와 바로 수정할 수 있습니다. **모델 표** 컬럼은 순번·모델명·제품그룹·브랜드·현재고·소비자가·지정가·입고가, **소모품 표** 컬럼은 순번·필터명·제품그룹·브랜드·규격·교체주기·현재고·소비자가입니다. 현재고가 안전재고 아래면 **빨간색**으로 표시됩니다.
>
> **하단 기능버튼 + 단축키**: 화면 하단 버튼 바에서 **신규(F2)·수정(F3)·삭제(F4)·저장(F5)·엑셀(F6)·닫기(Esc)** 를 쓸 수 있고, 그 아래 **①~⑤ 영역 설명**이 표시됩니다. 각 행의 **비활성** 버튼으로 소프트 삭제합니다.
...
- **② 적용 가능한 장비(모델)**: 필터 폼에서도 이 필터가 들어가는 **모델을 직접 지정**할 수 있습니다("+ 모델 추가" → 검색 드롭다운). 모델 화면(모델→필터)과 소모품 화면(필터→모델) 양쪽에서 연결을 관리할 수 있습니다.
