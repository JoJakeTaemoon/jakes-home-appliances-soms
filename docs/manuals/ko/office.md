...
**버전**: 2026-07-02 (대규모 업데이트 — 판매원·장비·방문·주문 도메인 반영)
**언어**: 한국어
...
### 5.5 보유 장비 탭 — 마스터-디테일 패널

![고객 장비 탭](../screenshots/ko/office/06-customer-equipment-tab.png)

좌측 장비 목록에서 장비 한 대를 클릭하면 우측에 해당 장비의 상세 정보가 펼쳐집니다.

**서비스 구성 통합표** — 점검 주기와 필터 교체 주기가 한 표에 표시됩니다:
...
- 마스터-디테일 패널: 목록 + 상세가 한 화면에서
- 서비스 구성 통합표: 점검 + 필터 스케줄이 한 표에
- 대량 등록 wizard: 3단계로 여러 장비를 한 번에 등록
...
**연결된 방문 이력** 섹션도 같은 페이지에 표시됩니다.

### 7.4 장비 대량 등록 Wizard (신규)

**사이드바 → 장비 → 대량 등록**

![대량 등록 1단계](../screenshots/ko/office/17-bulk-register-step1.png)

여러 장비를 한 번에 등록하는 3단계 wizard입니다.

#### 1단계: 고객 및 계약 선택

- 고객 검색 (코드 또는 이름)
- 연결할 계약 선택 (장비는 반드시 계약에 연결됩니다)
- B2B 다중 사업장 고객은 사업장도 선택

#### 2단계: 장비 정보 입력

표 형태로 한 행에 장비 하나씩 입력합니다:
- 모델 선택 (Brand → Model 드롭다운)
- 수량 (같은 모델 여러 대)
- 시리얼 번호 (자동 연번 또는 수동)
  - 숫자로 끝나는 시리얼 입력 시 (예: `PTS-2100-000010`) → 수량만큼 자동 채번 (`...000011`, `...000012`)
- 보증금, 월 요금 (모델 기본값 적용, 수정 가능)
- 서비스 유형: RENTAL / SALE / MAINTENANCE

#### 3단계: 확인 및 등록

입력 내용을 요약 확인 후 "**등록**" 버튼으로 일괄 저장합니다.
