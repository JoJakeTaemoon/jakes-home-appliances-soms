...
> **내보내기**: 화면 우측 상단에서 **카탈로그 CSV 다운로드** 또는 **Excel 내보내기**를 누르면 전체 카탈로그를 파일로 받을 수 있습니다. Excel 파일은 **Catalog**(모델×부품) + **Filters**(필터 마스터) 두 시트로 구성됩니다.

> **화면 구성(마스터-디테일)**: 모델·필터 탭은 **좌측 편집 폼 / 우측 목록** 구조입니다. 우측 목록에서 항목을 클릭하면 좌측 폼에 불러와 바로 수정할 수 있고, **"+ 신규"** 버튼으로 빈 폼(새 항목)이 됩니다. 상단 **검색**으로 목록을 좁힐 수 있으며, 각 목록 행의 **비활성** 버튼으로 소프트 삭제합니다.
...
#### 필터(소모품) 마스터 — 자기 정보만 관리

필터 관리 화면은 이제 그 **필터 자체의 정보만** 다룹니다: SKU · 이름(KO/VI/EN) · 교체 주기 · 판매가 · 비고. 어느 모델에 들어가는지는 위의 모델 화면에서 관리합니다.
