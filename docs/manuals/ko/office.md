...
- **관리번호(장비코드)** 는 등록을 마치는 순간 시스템이 자동 발행합니다. 직접 입력하는 방식은 없습니다.
  - 형식: **모델코드 + 설치일(YYMMDD) + 순번 4자리** — 예: `PTS21002609040001`
  - 순번은 **고객과 무관한 전역 순번**입니다. 같은 모델을 같은 날 다른 고객에게 설치해도 번호가 겹치지 않습니다.
