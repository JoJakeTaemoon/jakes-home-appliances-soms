...
**버전**: 2026-08-09 (보유 장비 탭 인라인 행 확장 + 제품 카탈로그 2단 레이아웃·번호 배지 + 입력값 관계 설명 추가)
**언어**: 한국어
...
**보유 장비 탭은 인라인 행 확장 방식입니다(개편).** 장비 목록에서 행을 클릭하면 별도 화면으로 이동하지 않고 **그 행 바로 아래에서 상세가 표 전체 너비로 펼쳐집니다**(아코디언). 다른 장비 행을 클릭하면 이전에 펼쳐져 있던 상세는 접히고 새로 클릭한 장비의 상세가 펼쳐지며, 펼쳐질 때 **약 0.4초의 부드러운 애니메이션**이 적용됩니다. 장비를 클릭해도 **화면 스크롤 위치는 그대로 유지**되어 위로 튀지 않습니다.

- **펼쳐진 상세 = 탭 5개**: 기본정보 · 필터정보 · 서비스이력 · 수금내역 · 메모. 기본정보 탭에 서비스 구성 통합표(아래)와 주요정보·다음 일정이 모여 있습니다. 표 전체 너비를 쓰기 때문에 이전의 좁은 우측 패널보다 한눈에 보기 편합니다.
- **펼친 상세 우상단의 "전체 화면 보기" 링크**: 이전·교체·해지·회수 같은 무거운 작업이 필요하면 이 링크로 기존 **전용 장비 상세 페이지**(§7.3)를 엽니다.
...
> 단, 고객 상세의 **보유 장비** 탭(§5.5)에서는 장비를 클릭해도 이 페이지로 바로 이동하지 않고 **행 아래에 상세가 인라인으로 펼쳐집니다**. 이 전용 페이지는 그 인라인 상세 우상단의 "전체 화면 보기" 링크로 열 수 있습니다.
...
> **화면 구성(마스터-디테일 + 번호 배지)**: 모델·소모품 탭은 **좌측 편집 폼 / 우측 목록(표)** 구조이며, 목업과 동일하게 화면 각 영역에 번호 배지 **①②③④⑤**가 붙습니다 — ① 모델정보/필터정보 ② 필터구성/적용모델 ③ 검색 ④ 목록 ⑤ 기능버튼. 우측 표에서 행을 클릭하면 좌측 폼에 불러와 바로 수정할 수 있습니다.
>
> **모델 폼(① 영역)은 2단 배치**입니다 — 좌측: 모델명·제품군·브랜드·모델설명·판매가, 우측: 현재고·소비자가·입고가·지정가. 그 아래 보조 영역에 월임대료·월관리비·안전재고·점검주기·보증기간·활성 여부가 이어집니다.
>
> **소모품 폼(① 영역)도 2단 배치**입니다 — 좌측: 필터명·제품군·브랜드·규격·주요용도, 우측: 교체주기·현재고·안전재고·소비자가·지정가·입고가. 맨 아래 **비고**는 전체 폭을 차지합니다.
>
> **모델 표(④ 영역)** 컬럼은 순번·모델명·제품그룹·브랜드·현재고·소비자가·지정가·입고가이고, 각 행 앞에 **체크박스**가 있으며 헤더에는 **전체선택** 체크박스가 있습니다. 여러 모델을 체크하면 **삭제(F4)** 로 한 번에 비활성화할 수 있고, 아무것도 체크하지 않으면 좌측 폼에 불러온 모델 1건만 대상이 됩니다. **소모품 표** 컬럼은 순번·필터명·제품그룹·브랜드·규격·교체주기·현재고·소비자가이며, **제품그룹**은 그 필터가 쓰이는 장비의 제품군을 따라 자동 표시됩니다. 현재고가 안전재고 아래면 **빨간색**으로 표시됩니다.
>
> **하단 기능버튼(⑤ 영역) + 단축키**: 화면 하단 버튼 바에서 **신규(F2)·수정(F3)·삭제(F4)·저장(F5)·엑셀로 저장(F6)** 을 쓸 수 있고, **소모품(필터) 화면에는 추가로 보고서 출력(F7)** 버튼이 있습니다. **닫기(Esc)** 로 편집을 마칩니다. 각 행의 **비활성** 버튼으로 소프트 삭제할 수도 있습니다.
...
- **② 적용 가능한 장비(모델)가 표로 변경**: 드롭다운에서 모델을 고른 뒤 **"추가"** 버튼을 누르면 아래 **표**(순번·모델명·제품군·브랜드·수량·삭제)에 쌓입니다(이전의 인라인 나열 방식 대체). 모델 화면(모델→필터)과 소모품 화면(필터→모델) 양쪽에서 연결을 관리할 수 있으며, 어느 쪽에서 추가하든 **양쪽에 함께** 반영됩니다.

#### 재고 조정 (입고·출고·조정)

제품·소모품 폼의 **현재고** 옆 **재고 조정** 버튼(수정 모드)에서 재고를 직접 움직입니다.
