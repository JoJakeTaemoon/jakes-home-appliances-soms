...
![로그인](../screenshots/ko/field/01-login.png)
...
| **🟡 서명 받을 서류 배지** | **(Phase 6)** 그 방문에서 손님 서명 필요한 서류 종류 (예: "장비 인수증 · 계약서"). 출발 전에 어떤 종이를 챙겨야 하는지 한눈에 확인 |

> 💡 **서명 받을 서류 배지**: 사무실이 그날 분을 "일괄 인쇄" 로 묶어서 미리 인쇄해두면 종이는 책상에서 받습니다. 배지는 "이 종이가 빠지면 안 된다" 는 체크리스트로 활용하세요. 못 챙겼으면 손님 댁 도착 후에도 모바일에서 미리보기 → 손님께 보여드리고 사무실에 인쇄 부탁할 수 있어요.
...
![방문 상세 — 서명 받을 서류 미리보기](../screenshots/ko/field/05-visit-detail-signature-docs.png)

| 항목 | 설명 |
|---|---|
| **방문 종류 + 상태** | 화면 상단 |
| **손님 정보** | 이름, 회사명 (B2B) |
| **연락처** | 일상 담당자 이름 + "전화" 버튼 (사무실 경유로 안내됨 — 손님 번호 직접 노출 안 됨) |
| **주소 + 지도 보기** | 길찾기 앱 연동 |
| **시간대** | 약속 시간 |
| **방문 사유** | "정기 필터 교체", "고장 신고: 물이 안 나옴" 등 |
| **장비 목록** | 이번 방문에서 작업할 장비들 (시리얼 번호 포함) |
| **🟡 서명 받을 서류** | **(Phase 6)** 각 서류의 **PDF 미리보기 (iframe)** + 서명자 안내. INSTALLATION 방문이면 **계약서 PDF 링크**도 함께 표시. 손님에게 "여기 사인해 주세요" 라고 가리킬 때 미리 확인 |
...
1. 출발 전 미리보기에서 종이 빠진 게 없는지 확인 (사무실 일괄 인쇄 묶음과 대조)
2. 손님 댁 도착 시 화면을 보여드리며 "이 종이의 이 위치에 서명 부탁드립니다" 안내
3. 미리보기를 보고도 종이가 없거나 모양이 다르면 사무실에 즉시 전화 → 재인쇄 또는 다른 기사 편으로 받음

> ⚠️ **모바일에서 직접 인쇄 안 됩니다.** 미리보기는 "오늘 가져갈 종이 형식 + 손님 안내" 용도. 인쇄는 사무실 또는 본인 모바일 `/f/{locale}/visits/print?date=오늘` 페이지에서.
...
![내 정보](../screenshots/ko/field/04-profile.png)
