...
**Status (2026-09-07, 2026-09-04 규칙 대체):** ✅ RESOLVED — **`MAY-{NNNNNN}`** (예: `MAY-000001`).

- **형식**: 고정 접두사 `MAY-` + 6자리 순번. 모델과 무관하게 접두사는 항상 동일합니다.
- **순번 범위**: **모델별**입니다. 각 모델이 `MAY-000001` 부터 독립적으로 올라갑니다.
- **모델 간 중복 허용**: 서로 다른 모델에 같은 번호가 존재합니다 (모델 A 와 모델 B 둘 다 `MAY-000001` 을 가짐). 따라서 **코드 문자열만으로는 장비를 특정할 수 없고**, 유니크한 것은 `(modelId, assetCode)` 쌍입니다 — 코드로 조회할 때는 모델을 함께 지정해야 합니다.
- **모델 내 중복 금지**: 한 모델 안에서는 번호가 겹치지 않습니다. DB `@@unique([modelId, assetCode])` 로 강제합니다.
- **고객 무관**: 고객이 달라도 같은 모델이면 그 모델의 단일 순번을 이어받습니다. 고객·사업장·날짜별로 번호가 초기화되지 않습니다.
- **발행 시점**: 고객에게 배정될 때. `Equipment.customerId` 가 필수라 등록 = 배정이며, 대량등록 / 다중라인 / 단건 등록 **구분 없이 동일 규칙**입니다. 직접 입력 방식은 없습니다.
- **비카탈로그 기기**(`modelId = null`, 타사 기기 유지관리)는 하나의 순번을 공유합니다. Postgres 가 NULL 을 서로 다른 값으로 취급해 위 복합 제약이 이 버킷을 막지 못하므로, 채번기의 advisory lock 이 유일성을 담보합니다.
- **수정 불가**: 한 번 발행되면 변경할 수 없습니다 (`PATCH /api/equipment/:id` 가 받지 않음). 장비 수정 화면에서는 읽기 전용으로 표시됩니다.
- **재사용 없음**: A.3 에 따라 폐기·회수 시에도 결번 처리하지 않고 상태만 바꿉니다.
- 시리얼 번호(`serialNumber`)는 기기에 인쇄된 제조사 번호로, 유니크하지 않은 **별도 필드**입니다.

**이전 규칙 (2026-09-04, 폐기):** `{modelCode}{YY}{MM}{DD}{NNNN}` — 설치일 기반 + 테이블 전체 유니크. 마이그레이션 `20260907000000_equipment_asset_code_per_model` 이 기존 행을 모델별로 재발행합니다.
