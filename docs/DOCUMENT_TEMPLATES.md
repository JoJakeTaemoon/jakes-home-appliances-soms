...
> **This body is fixed by the carrier.** It is the one CSKH content eSMS has registered for brandname `JAKE'S HOME APPLIANCES` (ViHAT sheet "ZBS Seoul"). The draft below it — the `[JakeApp] {date} {time} ...` wording this section used to carry — was never registered and must not be sent. Rewording anything here, including replacing the hotline with `{hq_phone}`, puts the message out of step with the approval.

| Lang | Template body | Char count |
|---|---|---:|
| VI | `TB BAO TRI DINH KY: KTV cua JAKE'S HOME APPLIANCES du kien se den bao tri {equipment} cua QK vao {datetime}. Neu QK can doi khung gio khac vui long LH: 0768902009.` | 130 + vars (GSM-7) |
| EN | `MAINTENANCE NOTICE: JAKE'S HOME APPLIANCES technician will service your {equipment} on {datetime}. To reschedule, please contact 0768902009.` | 107 + vars (GSM-7) |
...
- **No `[JakeApp]` prefix.** The brandname is already the sender ID on the handset, so repeating it in the body wastes characters.
