...
describe("POST /api/equipment/bulk-register — 4-step wizard", () => {
  it("(a) RENTAL: N units → N equipment + N visits + 1 contract (deposit=Σ, totalValue=Σrent×term)", async () => {
    ...
    // 장비코드 is system-issued as MAY-{NNNNNN}, consecutive within the batch
    // and sequenced per model (not per customer, not date-derived).
    const codes = (
    ...
    expect(codes.every((c) => /^MAY-\d{6}$/.test(c ?? ""))).toBe(true);
    expect(new Set(codes).size).toBe(3);
    const seq = codes.map((c) => Number(c!.slice("MAY-".length)));
