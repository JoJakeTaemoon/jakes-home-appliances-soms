...
- **제품군 is the model's only classifier (2026-09-24)**: the legacy `EquipmentCategory` enum (`WATER_PURIFIER` / `BIDET` / `AIR_PURIFIER` / `FILTER` / `OTHER`) and `EquipmentModel.category` were **dropped** in migration `20260924000000_drop_equipment_category_enum`; the migration backfills `categoryId` by matching `ProductCategory.code` to the old enum value (and mints an `OTHER` 제품군 only if some model still needs one). Models and consumables both classify through `EquipmentModel.categoryId` / `Consumable.categoryId` → the office-editable `ProductCategory` master (14 seeded codes, CSV import can add more). `categoryId` stays nullable in Prisma because the FK is `onDelete: SetNull`; the office form marks it required. Label everywhere is **제품군 / Nhóm sản phẩm / Category** (`equipmentModels.category` was "카테고리" until this change). Render names with `pickCategoryName(cat, locale)` from `src/lib/products/name.ts`; `categoryAltNames()` supplies the other two locales as a Combobox `description` so a ko-UI user can still find a 제품군 by typing accent-free Vietnamese. 제품군·브랜드 dropdowns (model form, consumable form) are searchable and carry an inline "+ 추가" row wired to `src/components/products/catalog-quick-create.tsx`, which POSTs to the existing catalog APIs (MANAGER+) and selects the new row in place; category codes come from `categoryCodeFromName()` in `src/lib/products/category-code.ts`, shared with the CSV importer.
